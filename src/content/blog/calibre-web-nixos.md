...
- For reading, either use Calibre to transfer the ebook directly to my Kobo
...
ebook collection that synchronizes the books, progress, and highlights between
...
I'd also prefer to manage the files more directly. EPUB files store their data
...
Some of them offer readers via web apps, but the web readers are usually clunky.
It seems like most users choose use these tools to self-host an
[OPDS](https://opds.io) server, reading the books with an app that offers OPDS
support. It is easy to find mobile apps that use OPDS, and my ereader can
participate as it's running [KOReader](https://koreader.rocks) which has OPDS
support. This doesn't synchronize highlights or progress, but it's a start.
Maybe this can be done by OPDS clients rather than the server.
...
connections from all of my network interfaces include the
...
`http://vexhalia:8083`. In the future I can make this nicer by putting
...
files to the server. I can run this periodically to update the collection on the
server. In the future, it might be nice to set up a cron job to do this
automatically and manage backups.
